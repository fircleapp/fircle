...
- **Channel-agnostic event model**: Notification records should be source-event oriented (tag, mention, invite/system) and leave transport concerns to future phases.
...
- [ ] Add notification enums and models in [prisma/schema.prisma](prisma/schema.prisma), including:
  - [ ] Notification category and event-type enums.
  - [ ] Notification entity linked to recipient FamilyMember and optional actor/context entities.
  - [ ] Read-state fields (isRead/readAt) and createdAt ordering fields.
- [ ] Add platform support models required for future channel rollout:
  - [ ] NotificationPreference entity (stored defaults, no UI wiring yet).
  - [ ] NotificationDeliveryLog (or equivalent) for channel/status attempt tracking.
- [ ] Add relation fields and indexes optimized for:
  - [ ] unread count by recipient member.
  - [ ] newest-first notification listing.
  - [ ] event correlation and delivery log lookup.
- [ ] Create migration under [prisma/migrations](prisma/migrations) and regenerate client in [generated/prisma](generated/prisma).
